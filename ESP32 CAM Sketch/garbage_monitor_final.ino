...
const char* password = "heyyyyyy";

const char* server = "192.168.43.204";
const int port = 5000;
...
void sendPhoto() {
  ...
  if (client.connect(server, port)) {
    ...
    const char *boundary = "14737809831466499882746641449";
    String header = "--" + String(boundary) + "\r\nContent-Disposition: form-data; name=\"image\"; filename=\"image.jpg\"\r\nContent-Type: image/jpeg\r\n\r\n";
    String footer = "\r\n--" + String(boundary) + "--\r\n";
    uint32_t imageLen = fb->len;
    uint32_t totalLen = imageLen + header.length() + footer.length();;
  ...
  else Serial.println("Server connection failed");
}
